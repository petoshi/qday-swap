...
func startIntegrationNode(t *testing.T) *integrationNode {
	...
	if binary == "" {
		t.Skip("LITECOIND is required")
	}
	port := freePort(t)
	n := &integrationNode{t: t, binary: binary, dataDir: t.TempDir(), url: fmt.Sprintf("http://127.0.0.1:%d", port)}
	log, err := os.OpenFile(filepath.Join(n.dataDir, "litecoin.log"), os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0600)
	if err != nil {
		t.Fatal(err)
	}
	n.log = log
	...
	n.close()
	t.Fatal("litecoind did not become ready")
	return nil
}
...
func (n *integrationNode) createWallet(t *testing.T, name string) *Client {
	t.Helper()
	admin := n.client(t, "")
	if err := admin.call(context.Background(), false, nil, "createwallet", name); err != nil {
		t.Fatal(err)
	}
...
func TestProductionLitecoinAdapter(t *testing.T) {
	node := startIntegrationNode(t)
	funder := node.createWallet(t, "funder")
	recipientWallet := node.createWallet(t, "recipient")
	observer := node.createWallet(t, "observer")
	...
	contract, err := NewContract(LitoshisPerLitecoin, hash, recipientKey.PubKey().SerializeCompressed(), refundKey.PubKey().SerializeCompressed(), height+10)
	if err != nil {
		t.Fatal(err)
	}
